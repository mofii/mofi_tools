function mofi_plot_samples (root_dir, varargin)
% mofi_plot_samples (root_dir, varargin)
%
% Input: 
...
if ~exist(root_dir, 'dir'), error('Input ''root_dir'' must exist.'); end
...
for idx = 1:length(folders)
    ...
    set(gca, 'XTick',[1 round(size(samples,2)/2) size(samples,2)])
