...
function [angles] = mofi_setup_3d_lines (varargin)

%
% Sets up the angle of each transmit and receive line. (Mostly for explososcan)
% 
% [angles angles_tx angles_rx] = mofi_setup_3d_lines (varargin)
%
% 
% -- Input Arguments --
...
% MFR 2013-08-05 - Renamed from mfr_explosolines_setup to mofi_setup_3d_lines.
%
...
if no_TX_zx == 1 && length(span_zx) ==2
    span_zx = mean(span_zx(:));
elseif length(span_zx) ~=2
    error('span_zx must contain 2 elements.')
end

if no_TX_zy == 1 && length(span_zy) ==2
        span_zy = mean(span_zy(:));
elseif length(span_zy) ~=2
    error('span_zy must contain 2 elements.')
end
...
% X
num_tx_zy     = length(tx_rot_zy);
rx_zy         = cell(num_tx_zy,1);
rx_zy_dist    = cell(num_tx_zy,1);
rx_zy_idx     = cell(num_tx_zy,1);
rx_zy_overlap = cell(num_tx_zy,1);
for tx_idx = 1:num_tx_zy
...
% Y
num_tx_zx     = length(tx_rot_zx);
rx_zx         = cell(num_tx_zx,1);
rx_zx_dist    = cell(num_tx_zx,1);
rx_zx_idx     = cell(num_tx_zx,1);
rx_zx_overlap = cell(num_tx_zx,1);
