...
% 2013-09-16 Renamed to from mfr_window to mofi_window. MFR
%


function win = mofi_window (win_name, n, a)
%% Sanity check
...
if (nargin >2 && sum(isnan(a))>0),     error('a contains NaN');end
if ~ischar(win_name),                  error('Input argument "win_name" must be a string.');end
if ~isnumeric(a) || length(a)>1        error('Input argument "a" must be a scalar.');end

% Make it case in-sensitive.
win_name = lower(win_name);
